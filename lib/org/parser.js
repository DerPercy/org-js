...
Parser.prototype = {
  ...
  parseElement: function () {
    ...
    case Lexer.tokens.directive:
      ...
      break;
		case Lexer.tokens.scheduled:
	    element = this.parseScheduled();
	    break;
		case Lexer.tokens.properties:
		  element = this.parseProperties();
		  break;
  ...
  parseHeader: function () {
	...
	parseScheduled: function () {
		var schedToken = this.lexer.getNextToken();
		//console.log(schedToken)
		var sched = Node.createScheduled([],schedToken);
  ...
  // ------------------------------------------------------------
	parseProperties: function () {
		...
		while (this.lexer.hasNext()) {
			...
			if(nextToken.type === Lexer.tokens.propertiesEnd){
				break;
			}
		}
		var props = Node.createProperties([]);
