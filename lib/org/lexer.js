...
Syntax.define("scheduled", /^((DEADLINE:).*[<](.*)[>].*)?((SCHEDULED:).*[<](.*)[>].*)?$/);
Syntax.define("propertiesStart", /^(:PROPERTIES:).*$/);
Syntax.define("propertiesEnd", /^(:END:).*$/);
...
Lexer.prototype = {
  tokenize: function (line) {
    ...
    if (Syntax.isHeader(line)) {
      ...
      token.indentation = RegExp.$1.length;
      token.content     = RegExp.$2;
    } else if (Syntax.isScheduled(line)) {
      token.type        = Lexer.tokens.scheduled;
			//console.log(RegExp.$6)
      //token.indentation = RegExp.$1.length;
      //token.content     = RegExp.$2;
			token.scheduled = RegExp.$6
			token.deadline = RegExp.$3
    } else if (Syntax.isPropertiesStart(line)) {
      token.type        = Lexer.tokens.properties;
      token.indentation = RegExp.$1.length;
      token.content     = RegExp.$2;
    } else if (Syntax.isPropertiesEnd(line)) {
      token.type        = Lexer.tokens.propertiesEnd;
      token.indentation = RegExp.$1.length;
      token.content     = RegExp.$2;
